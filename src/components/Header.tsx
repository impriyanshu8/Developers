...
const Header = () => {
  ...
  useEffect(() => {
    const calculateVisibleItems = () => {
      ...
      // Adjusted calculations for better responsiveness at 1200px
      const baseItemWidth = 95; // Reduced base width
      const moreButtonWidth = 70; // Reduced more button width
      ...
      // Dynamic thresholds based on screen width
      ...
      const itemsToShow = Math.max(minItems, Math.min(maxItems, maxItemsAllowed, navigationItems.length));
    ...
    <header className={`bg-white/95 backdrop-blur-md shadow-sm sticky top-0 z-50 transition-all duration-300 ${isScrolled ? 'shadow-lg' : ''}`}>
      ...
      <div className="w-full mx-auto px-2 sm:px-4 lg:px-6">
        <div className="flex justify-between items-center py-3 sm:py-4">
          ...
          <nav ref={navRef} className="hidden lg:flex items-center space-x-0.5 xl:space-x-1 flex-1 justify-center mx-2 xl:mx-4">
            ...
            {visibleNavItems.map((item) => (
              ...
              >
                ...
                {item.dropdown && activeDropdown === item.name && (
                  <div className="absolute top-full left-0 mt-1 w-64 bg-white rounded-xl shadow-xl border border-gray-200 z-50 py-2 opacity-0 translate-y-2 animate-[fadeInUp_0.2s_ease-out_forwards] before:content-[''] before:absolute before:-top-1 before:left-0 before:w-full before:h-1 before:bg-transparent">
            ...
            {hiddenNavItems.length > 0 && (
              <div 
                ...
                onMouseLeave={() => setActiveDropdown(null)}
              >
                <button className="flex items-center px-2 xl:px-3 py-2 text-xs xl:text-sm text-gray-700 hover:text-blue-600 hover:bg-blue-50 font-medium rounded-lg transition-all duration-200 whitespace-nowrap">
                  <MoreHorizontal className="h-4 w-4 mr-1" />
                  More
                </button>
                {activeDropdown === 'more' && (
                  <div className="absolute top-full right-0 mt-1 w-56 bg-white rounded-xl shadow-xl border border-gray-200 z-50 py-2 opacity-0 translate-y-2 animate-[fadeInUp_0.2s_ease-out_forwards] before:content-[''] before:absolute before:-top-1 before:right-4 before:w-16 before:h-1 before:bg-transparent">
