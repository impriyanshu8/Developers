...
const Header = () => {
  ...
  return (
    ...
    >
      {/* Government Info Bar */}
      <div className="bg-[#118DC4] text-white py-1 sm:py-2">
          ...
          <div className="flex justify-between items-center text-xs sm:text-sm text-gray-700">
            <div className="flex items-center space-x-2 sm:space-x-4 min-w-0">
              <a
                ...
                className="flex items-center hover:text-[#118DC4] transition-colors flex-shrink-0"
              >
        ...
        <div className="flex justify-between items-center py-2 sm:py-3 lg:py-4">
          ...
          <div className="flex items-center space-x-2 sm:space-x-3 flex-shrink-0 min-w-0 max-w-[60%] sm:max-w-none">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="text-xs sm:text-sm text-[#118DC4] font-medium truncate">
          ...
          <nav
            ref={navRef}
            className="hidden lg:flex items-center space-x-0.5 flex-1 justify-center mx-1"
          >
            ...
            {visibleNavItems.map((item) => (
              ...
              >
                <button
                  className={`flex items-center px-1.5 py-2 transition-all duration-200 font-medium hover:bg-[#E6F4FB] rounded-lg text-xs whitespace-nowrap ${
                    isActivePath(item.href) ||
                    (item.dropdown && hasActiveDropdownItem(item.dropdown))
                      ? "text-[#118DC4] bg-[#E6F4FB] shadow-sm"
                      : "text-gray-700 hover:text-[#118DC4]"
                  }`}
                >
                  <span className="truncate max-w-[80px]">{item.name}</span>
                  {item.dropdown && (
                    <ChevronDown className="ml-1 h-3 w-3 flex-shrink-0 transition-transform duration-200 group-hover:rotate-180" />
                  )}
                </button>
                      ...
                      <button
                        ...
                        className={`block w-full text-left px-4 py-3 text-sm transition-all duration-150 border-b border-gray-100 last:border-b-0 hover:pl-6 ${
                          isActivePath(subItem.href.split("#")[0])
                            ? "bg-[#E6F4FB] text-[#118DC4] font-medium border-l-4 border-l-[#118DC4]"
                            : "text-gray-700 hover:bg-[#E6F4FB] hover:text-[#118DC4]"
                        }`}
                      >
                        {subItem.name}
                      </button>
            ...
            {hiddenNavItems.length > 0 && (
              <div
                ...
                onMouseLeave={() => setActiveDropdown(null)}
              >
                <button className="flex items-center px-1.5 py-2 text-xs text-gray-700 hover:text-[#118DC4] hover:bg-[#E6F4FB] font-medium rounded-lg transition-all duration-200 whitespace-nowrap">
                            ...
                            {item.dropdown.map((subItem) => (
                              <button
                                ...
                                className={`block w-full text-left px-4 py-2 text-xs transition-all duration-150 border-b border-gray-100 last:border-b-0 hover:pl-6 ${
                                  isActivePath(subItem.href.split("#")[0])
                                    ? "bg-[#E6F4FB] text-[#118DC4] font-medium"
                                    : "text-gray-600 hover:bg-[#E6F4FB] hover:text-[#118DC4]"
                                }`}
                              >
                                {subItem.name}
                              </button>
          ...
          <div className="flex items-center space-x-1 sm:space-x-2 lg:space-x-3 flex-shrink-0">
            <Button className="hidden sm:block bg-[#118DC4] hover:bg-[#0F7AB0] text-white font-medium text-xs lg:text-sm px-2 sm:px-3 lg:px-4 py-1.5 sm:py-2 rounded-lg shadow-lg hover:shadow-xl transition-all duration-300 transform hover:scale-105">
            ...
            <Button
              variant="ghost"
              size="sm"
              className="lg:hidden p-1.5 sm:p-2 hover:bg-[#E6F4FB] transition-colors"
          ...
          <div className="lg:hidden border-t bg-white/95 backdrop-blur-md shadow-lg rounded-b-xl mb-2 max-h-[80vh] overflow-y-auto">
            <nav className="py-3 sm:py-4 space-y-1">
                      ...
                      {item.dropdown.map((subItem) => (
                        <button
                          ...
                          className={`block w-full text-left px-3 sm:px-4 py-2 text-sm transition-all duration-150 rounded ${
                            isActivePath(subItem.href.split("#")[0])
                              ? "text-[#118DC4] bg-[#E6F4FB] font-medium"
                              : "text-gray-600 hover:text-[#118DC4] hover:bg-[#E6F4FB]"
                          }`}
                        >
                          {subItem.name}
                        </button>
              ...
              <div className="px-3 sm:px-4 pt-4">
                <Button className="w-full bg-[#118DC4] hover:bg-[#0F7AB0] text-white font-medium text-sm shadow-lg">
                  Apply Online
                </Button>
              </div>
            </nav>
